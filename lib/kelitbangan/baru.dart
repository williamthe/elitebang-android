...
class FormBaru extends StatefulWidget{
  ...
  @override
  State<FormBaru> createState() => _FormBaruState(bidang);
}
...
class _FormBaruState extends State<FormBaru> {
  ...
  String pelaksana = '';
  int dokumen = 0;
  _FormBaruState(int bidang){
    this.instansi = bidang;
  }
  ...
  getNomor() async {
    await http.get(
      Uri.https(env.apiUrl, 'api/kelitbangan/nomor'),
      headers: {
  ...
  getFileRangkuman() async {
    ...
    if (result != null) {
      ...
      rangkuman = pickFile.toString().replaceAll(' ', '-');
