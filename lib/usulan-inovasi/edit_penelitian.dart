...
class EditUsulan extends StatefulWidget{
    Map myData = {};
    EditUsulan(Map data){
      myData = data;
    }
  _EditUsulan createState() => _EditUsulan(myData);
}

class _EditUsulan extends State<EditUsulan>{

  Map myData = {};
  _EditUsulan(Map data){
    myData = data;
  }
...
class _FormEditState extends State<FormEdit> {
  ...
  getInstansi() async {
    await http.get(
      ...
      setState(() {
        instansi = myData['instansi'] == null ? 0 : myData['instansi'];
      });
  ...
  Future<void> update() async {
    ...
    await http.post(
      Uri.https(env.apiUrl, 'api/usulan-inovasi/update'),
      headers: {
  ...
  Future<void> updateStatus(status) async {
    ...
    await http.post(
      Uri.https(env.apiUrl, 'api/usulan-inovasi/update-status'),
      headers: {
        //'Content-Type' : 'aplication/json;',
        'Authorization': 'Bearer ${_mytoken.toString()}'
      },
  ...
  Future<void> delete() async {
    ...
    var _mytoken = await SessionManager().get('_token');
     ...
     await http.post(
      Uri.https(env.apiUrl, 'api/usulan-inovasi/delete'),
      headers: {
        //'Content-Type' : 'aplication/json;',
        'Authorization': 'Bearer ${_mytoken.toString()}'
      },
  ...
  confirmDelete(){
    showDialog(context: context, builder: (BuildContext conf){
      updel = conf;
      return AlertDialog(
        title: Text( 'Akan Menghapus Data?',style: TextStyle() ),
        actions: [
          TextButton(onPressed: (){
            Navigator.of(updel).pop();
            delete();
