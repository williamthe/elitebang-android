...
class FormEditKeluar extends StatefulWidget{
  Map dataKelitbangan = {};
  FormEditKeluar(Map data){
    this.dataKelitbangan = data;


  }
  @override
  State<FormEditKeluar> createState() => _FormEditKeluarState(dataKelitbangan);
}

class _FormEditKeluarState extends State<FormEditKeluar> {
  ...
  _FormEditKeluarState(Map myData){
    this.myData     = myData;
    this.id         = myData['id'].toString()  == null ? '0' : myData['id'].toString();
    this.nomor_surat= myData['nomor_surat']        == null ? '-' : myData['nomor_surat'].toString().substring(4);
    ...
    this.namaFile   = myData['surat_keluar']        == null ? '-' : myData['surat_keluar'];
        ...
        child: Row(
          children: [
            ...
            Expanded(
                flex: 1,
                child: IconButton(onPressed: (){
                  launchUrlString('${env.webUrl}/files-attachment/surat-keluar/${namaFile}',mode: LaunchMode.externalApplication);
  ...
  Future<void> update() async {
    ...
    await http.post(
      Uri.https(env.apiUrl, 'api/surat-keluar/update'),
      headers: {
  ...
  Future<void> delete() async {
    ...
    await http.post(
      Uri.https(env.apiUrl, 'api/surat-keluar/delete'),
      headers: {
        'Authorization': 'Bearer ${_mytoken.toString()}'
      },
      body: {
