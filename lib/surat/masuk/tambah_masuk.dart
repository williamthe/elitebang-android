...
class FormTambahMasuk extends StatefulWidget{
  @override
  State<FormTambahMasuk> createState() => _FormTambahMasukState();
}

class _FormTambahMasukState extends State<FormTambahMasuk> {
  ...
  getKlasifikasi() async {
    ...
    await http.get(
      ...
      for(var ix = 0; ix< listInstansi.length; ix++) {
        kodeKlasifikasi[ix] = listInstansi[ix]['kode'];
      }
  ...
  Future<void> simpan() async {
    ...
    await http.post(
      Uri.https(env.apiUrl, 'api/surat-masuk/create'),
      headers: {
  ...
  getFile() async {
    ...
    if (result != null) {
      ...
      namaFile = pickFile.name.toString().replaceAll(' ', '-');
      childFile.clear();
  ...
  uploadFile() async{
    var request = http.MultipartRequest("POST",Uri.https(env.webPoint, '/upload-surat-masuk'));
