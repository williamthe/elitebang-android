...
class _FormEditState extends State<FormEdit> {
        ...
        child: Row(
          children: [
            ...
            Expanded(
                flex: 1,
                child: IconButton(onPressed: (){
                  launchUrlString('${env.webUrl}/files-attachment/regulasi/${namaFile}',mode: LaunchMode.externalApplication);
                ...
                attachWidget.forEach((key, value) {
                  ...
                  attachment.add({
                    'nama' : namaFile.toString().replaceAll(" ", "-"),
                     'url':  '/files-attachment/regulasi/'+key.toString().replaceAll(" ", "-"),
                    'tipe': 'application',
                  });
                });
  ...
  Future<void> update() async {
    ...
    await http.post(
      Uri.https(env.apiUrl, 'api/regulasi/update'),
      headers: {
  ...
  Future<void> delete() async {
    ...
    showDialog(context: context, builder: (BuildContext upd){
      ...
      return AlertDialog(
        title: Column(
          children: [
            Text( 'Menghapus Data...', style: TextStyle() ),
    ...
    await http.post(
      Uri.https(env.apiUrl, 'api/regulasi/delete'),
      headers: {
        'Authorization': 'Bearer ${_mytoken.toString()}'
      },
      body: {
  ...
  uploadFile() async{
    var request = http.MultipartRequest("POST",Uri.https(env.apiUrl, '/upload-regulasi'));
